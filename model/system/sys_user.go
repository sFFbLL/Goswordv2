...
type SysUser struct {
	...
	HeaderImg   string         `json:"headerImg" gorm:"not null;default:http://http://r9qsta3s9.hn-bkt.clouddn.com/head.png;comment:用户头像"` // 用户头像
	Email       string         `json:"email" gorm:"not null;comment:用户邮箱"`
	DeptId      uint           `json:"deptId" gorm:"not null "`
