...
type GzlRecord struct {
	...
	CurrentState uint8          `json:"currentState" gorm:"not null;default:1;comment:当前状态(进行中1默认、已完成2、已结束3)"` //当前状态(进行中1默认、已完成2、已结束3)
	Form         utils.JSON     `json:"form" gorm:"not null;type:json;comment:表单JSON"`                         //表单JSON
	CurrentNode  string         `json:"currentNode" gorm:"comment:当前节点"`                                       //当前节点
	DeptId       uint           `json:"depId" gorm:"comment:部门id"`                                             //部门id
